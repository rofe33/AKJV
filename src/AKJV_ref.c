...
AKJV_newref()
{
    return calloc(1, sizeof(AKJV_ref));
}
...
static bool
AKJV_bookequal(const char *a, const char *b, bool short_match)
{
    for (size_t i = 0, j = 0; ; ) {
...
static bool
AKJV_book_matches(const AKJV_book *book, const char *s)
{
    return AKJV_bookequal(book->name, s, false) ||
        AKJV_bookequal(book->abbr, s, false) ||
        AKJV_bookequal(book->name, s, true);
}

static int
AKJV_book_fromname(const char *s)
{
    for (int i = 0; i < AKJV_books_length; i++) {
        const AKJV_book *book = &AKJV_books[i];
        if (AKJV_book_matches(book, s)) {
            return book->number;
        }
    }
    return 0;
...
static int
AKJV_scanbook(const char *s, int *n)
{
    int i;
    int mode = 0;
    for (i = 0; s[i]; i++) {
...
AKJV_parseref(AKJV_ref *ref, const char *ref_str)
{
    ...
    int n = 0;
    if (AKJV_scanbook(ref_str, &n) == 1) {
        // 1, 2, 3, 3a, 4, 5, 6, 8, 9
        char *bookname = strndup(ref_str, n);
        ref->book = AKJV_book_fromname(bookname);
        free(bookname);
        ref_str = &ref_str[n];
    } else if (ref_str[0] == '/') {
        // 7
